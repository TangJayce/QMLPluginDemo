...
#include "MozaTreeModelToTableModel.h"
#include <cmath>
...
QAbstractItemModel *MozaTreeModelToTableModel::model() const
{
    return m_model;
}

void MozaTreeModelToTableModel::setModel(QAbstractItemModel *arg)
{
    struct Cx {
        const char *signal;
        const char *slot;
...
void MozaTreeModelToTableModel::clearModelData()
{
    beginResetModel();
    m_items.clear();
...
QModelIndex MozaTreeModelToTableModel::rootIndex() const
{
    return {m_rootIndex};
}

void MozaTreeModelToTableModel::setRootIndex(const QModelIndex &idx)
{
    if (m_rootIndex == idx)
        return;
...
void MozaTreeModelToTableModel::resetRootIndex()
{
    setRootIndex(QModelIndex());
}

QModelIndex MozaTreeModelToTableModel::index(int row, int column, const QModelIndex &parent) const
{
    return hasIndex(row, column, parent) ? createIndex(row, column) : QModelIndex();
}

QModelIndex MozaTreeModelToTableModel::parent(const QModelIndex &child) const
{
    Q_UNUSED(child)
    return {};
}

QHash<int, QByteArray> MozaTreeModelToTableModel::roleNames() const
{
    if (!m_model) { return {}; }
    return m_model->roleNames();
}

int MozaTreeModelToTableModel::rowCount(const QModelIndex &) const
{
    if (!m_model) { return 0; }
    return m_items.size();
}

int MozaTreeModelToTableModel::columnCount(const QModelIndex &parent) const
{
    if (!m_model) { return 0; }
    return m_model->columnCount(parent);
}

QVariant MozaTreeModelToTableModel::data(const QModelIndex &index, int role) const
{
    if (!m_model) { return {}; }
...
bool MozaTreeModelToTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (!m_model)
        return false;
...
QVariant MozaTreeModelToTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    return m_model->headerData(section, orientation, role);
}

Qt::ItemFlags MozaTreeModelToTableModel::flags(const QModelIndex &index) const
{
    return m_model->flags(mapToModel(index));
}

int MozaTreeModelToTableModel::depthAtRow(int row) const
{
...
int MozaTreeModelToTableModel::itemIndex(const QModelIndex &index) const
{
...
bool MozaTreeModelToTableModel::isVisible(const QModelIndex &index) const
{
    return itemIndex(index) != -1;
}

bool MozaTreeModelToTableModel::childrenVisible(const QModelIndex &index)
{
    return (index == m_rootIndex && !m_items.isEmpty())
           || (m_expandedItems.contains(index) && isVisible(index));
}

QModelIndex MozaTreeModelToTableModel::mapToModel(const QModelIndex &index) const
{
...
QModelIndex MozaTreeModelToTableModel::mapFromModel(const QModelIndex &index) const
{
    if (!index.isValid()) { return {}; }
...
QModelIndex MozaTreeModelToTableModel::mapToModel(int row) const
{
    if (row < 0 || row >= m_items.size()) { return {}; }
    return {m_items.at(row).index};
}

QItemSelection  MozaTreeModelToTableModel::selectionForRowRange(const QModelIndex &fromIndex, const QModelIndex &toIndex) const
{
    int from = itemIndex(fromIndex);
    int to = itemIndex(toIndex);
    if (from == -1) {
        if (to == -1) { return {}; }
        return {toIndex, toIndex};
    }
...
void MozaTreeModelToTableModel::showModelTopLevelItems(bool doInsertRows)
{
    if (!m_model)
        return;
...
void MozaTreeModelToTableModel::showModelChildItems(const TreeItem &parentItem, int start, int end, bool doInsertRows, bool doExpandPendingRows)
{
...
void MozaTreeModelToTableModel::expand(const QModelIndex &idx)
{
    if (!m_model)
        return;
...
void MozaTreeModelToTableModel::collapse(const QModelIndex &idx)
{
    if (!m_model)
        return;
...
bool MozaTreeModelToTableModel::isExpanded(const QModelIndex &index) const
{
    if (!m_model)
        return false;
...
bool MozaTreeModelToTableModel::isExpanded(int row) const
{
    if (row < 0 || row >= m_items.size())
        return false;
...
bool MozaTreeModelToTableModel::hasChildren(int row) const
{
    if (row < 0 || row >= m_items.size())
        return false;
...
bool MozaTreeModelToTableModel::hasSiblings(int row) const
{
    const QModelIndex &index = mapToModel(row);
    return index.row() != m_model->rowCount(index.parent()) - 1;
}

void MozaTreeModelToTableModel::expandRow(int n)
{
...
void MozaTreeModelToTableModel::expandRecursively(int row, int depth)
{
...
void MozaTreeModelToTableModel::expandPendingRows(bool doInsertRows)
{
...
void MozaTreeModelToTableModel::collapseRecursively(int row)
{
...
void MozaTreeModelToTableModel::collapseRow(int n)
{
    if (!m_model || !isExpanded(n))
        return;
...
int MozaTreeModelToTableModel::lastChildIndex(const QModelIndex &index) const
{
...
void MozaTreeModelToTableModel::removeVisibleRows(int startIndex, int endIndex, bool doRemoveRows)
{
...
void MozaTreeModelToTableModel::modelHasBeenDestroyed()
{
...
void MozaTreeModelToTableModel::modelHasBeenReset()
{
    clearModelData();
...
void MozaTreeModelToTableModel::modelDataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void MozaTreeModelToTableModel::modelLayoutAboutToBeChanged(const QList<QPersistentModelIndex> &parents, QAbstractItemModel::LayoutChangeHint hint)
{
    Q_UNUSED(hint)
...
void MozaTreeModelToTableModel::modelLayoutChanged(const QList<QPersistentModelIndex> &parents, QAbstractItemModel::LayoutChangeHint hint)
{
    Q_UNUSED(hint)
...
void MozaTreeModelToTableModel::modelRowsAboutToBeInserted(const QModelIndex & parent, int start, int end)
{
    Q_UNUSED(parent)
    Q_UNUSED(start)
    Q_UNUSED(end)
...
void MozaTreeModelToTableModel::modelRowsInserted(const QModelIndex & parent, int start, int end)
{
    TreeItem item;
...
void MozaTreeModelToTableModel::modelRowsAboutToBeRemoved(const QModelIndex & parent, int start, int end)
{
...
void MozaTreeModelToTableModel::modelRowsRemoved(const QModelIndex & parent, int start, int end)
{
    Q_UNUSED(start)
    Q_UNUSED(end)
    int parentRow = itemIndex(parent);
...
void MozaTreeModelToTableModel::modelRowsAboutToBeMoved(const QModelIndex & sourceParent, int sourceStart, int sourceEnd, const QModelIndex & destinationParent, int destinationRow)
{
    enableSignalAggregation();
...
void MozaTreeModelToTableModel::modelRowsMoved(const QModelIndex & sourceParent, int sourceStart, int sourceEnd, const QModelIndex & destinationParent, int destinationRow)
{
    if (!childrenVisible(sourceParent)) {
...
void MozaTreeModelToTableModel::dump() const
{
    if (!m_model)
        return;
...
bool MozaTreeModelToTableModel::testConsistency(bool dumpOnFail) const
{
    if (!m_model) {
        if (!m_items.isEmpty()) {
...
void MozaTreeModelToTableModel::enableSignalAggregation() {
    m_signalAggregatorStack++;
}

void MozaTreeModelToTableModel::disableSignalAggregation() {
    m_signalAggregatorStack--;
...
void MozaTreeModelToTableModel::queueDataChanged(const QModelIndex &topLeft,
...
void MozaTreeModelToTableModel::emitQueuedSignals()
{
